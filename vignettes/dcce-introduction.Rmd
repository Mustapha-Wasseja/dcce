...
## Long-Run Estimators: CS-DL and CS-ARDL

For researchers interested in long-run relationships, the `dcce` package implements
two estimators from @chudik2016long.

### CS-DL (Cross-Sectionally Augmented Distributed Lag)

The **CS-DL** estimator adds the first difference of $\mathbf{x}$ and its lags
as short-run regressors alongside the level. The coefficient on the **level** of
$\mathbf{x}$ directly identifies the long-run effect:

$$
\Delta y_{it} = \alpha_i + w_i x_{it} + \sum_{\ell=0}^{p_T} \phi_{i\ell} \Delta x_{i,t-\ell}
                + \text{CSA terms} + u_{it}.
$$

Here $w_i$ is unit $i$'s long-run coefficient; $\hat{w}^{\text{MG}}$ is the
mean-group average.
...
fit_csdl <- dcce(
  ...
  unit_index         = "country",
  time_index         = "year",
  formula            = d_log_rgdpo ~ log_ck + log_hc + log_ngd,
  model              = "csdl",
  cross_section_vars = ~ log_rgdpo + log_hc + log_ck + log_ngd,
  cross_section_lags = 3
)
...
The **CS-ARDL** estimator fits an ARDL$(p_y, p_x)$ model with CSAs and recovers
long-run coefficients via the delta method:

$$
\text{LR coefficient} = \frac{\sum_{\ell=0}^{p_x} \hat\gamma_\ell}{1 - \sum_{\ell=1}^{p_y} \hat\phi_\ell}.
$$
...
fit_csardl <- dcce(
  ...
  unit_index         = "country",
  time_index         = "year",
  formula            = d_log_rgdpo ~ L(log_rgdpo, 1) + log_ck + log_hc + log_ngd,
  model              = "csardl",
  cross_section_vars = ~ log_rgdpo + log_hc + log_ck + log_ngd,
  cross_section_lags = 3
)
...
```{r cd-all-tests}
# Run all four CD tests on DCCE residuals
